...
CloudTrail 이벤트를 실시간으로 수집·분석하여 사전 정의된 5가지 탐지 시나리오를 기반으로 이상 행위를 식별하고, Slack 또는 Microsoft Teams를 통해 보안 담당자에게 즉시 알림을 제공합니다.
...
- `ref-suspicious-detector` Lambda (Python 3.14) + DynamoDB Streams + Slack/Teams 알림
...
Streams, 알림 연동)를 AWS SAM 프로젝트로 구현한 `template.yaml`과 Lambda 소스(`src/`)를 포함하고
있습니다. Organization Trail이 없어도 자체 S3 버킷+CloudTrail로 엔드투엔드 테스트가 가능한 데모
모드를 지원하며, 알림 채널은 `NotificationProvider` 파라미터로 Slack과 Microsoft Teams 중
선택할 수 있습니다.
...
[docs/sam-deployment-guide.md](docs/sam-deployment-guide.md)를, 알림 채널별 설정(Slack App /
Teams Webhook 생성 등)은 [docs/notifications/slack.md](docs/notifications/slack.md) /
[docs/notifications/teams.md](docs/notifications/teams.md)를 참고하세요.
